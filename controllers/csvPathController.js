...
// Where Excel files will be stored publicly
const EXPORT_DIR = path.join(__dirname, '..', 'public', 'exports');
fs.mkdirSync(EXPORT_DIR, { recursive: true });

// Your public domain for building file links
...
// Controller: POST /api/tickets/export/link
export const createTicketsExcelLink = async (req, res) => {
  ...
  try {
    ...
    const fname = `tickets_${conductor_id}${from?`_from-${from}`:''}${to?`_to-${to}`:''}${trip?`_trip-${trip}`:''}_${stamp}_${nanoid(8)}.xlsx`;
